...
def datasource(directory, sequence):
	...
	field_template = dict(func='bold_rest/f*-01.nii',
                          struct='axial_mprage/s*-01.nii')
...
def resting(directory):
	...
	task.connect(task.get_node('datasource'),'func',merge,'in_files')
	task.connect(merge,'merged_file',task.get_node('preprocess'),'input.func')
